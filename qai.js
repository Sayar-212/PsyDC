...
function updateDisplay() {
    ...
    const savedScore = qaiData[patientId];
    document.getElementById('psygenScore').value = savedScore || '';
...
document.getElementById('saveBtn').addEventListener('click', () => {
    ...
    const score = document.getElementById('psygenScore').value;
    ...
    fetch(SHEET_URL, {
        ...
        body: JSON.stringify({
            ...
            aiScore: patient.ai_score,
            aiLevel: patient.ai_severity,
...
document.getElementById('psygenScore').addEventListener('input', (e) => {
    const score = parseInt(e.target.value);
    document.getElementById('severity').textContent = (score >= 0 && score <= 27) ? getSeverity(score) : '-';
});

document.getElementById('exportBtn').addEventListener('click', () => {
    alert('Data is automatically saved to Google Sheets. Check the QAI sheet in your spreadsheet.');
});
